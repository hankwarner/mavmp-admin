...
<script>
import { updateEmployee, getEmployeeByID, addImageUrlToDB, createBlobFromImage, deleteEmployeeById } from "@/services/EmployeeService";
...
export default {
    ...
    methods: {
        async getEmployee(){
            this.employee = await getEmployeeByID(this.employee.id);
        },
        async updateEmployee(){
			try {
				this.loadingForm = true;

                let employeeToUpdate = this.employee;
                // Remove the token on the image url before sending to db
                employeeToUpdate.image = employeeToUpdate.image.split('?')[0];

                this.apiResponse = updateEmployee(employeeToUpdate);

                this.$buefy.toast.open('Employee successfully updated!');
        ...
        async deleteEmployee(){
			try {
                this.$buefy.dialog.confirm({
                    ...
                    hasIcon: true,
                    onConfirm: async () => {
                        this.apiResponse = await deleteEmployeeById(this.employee);

                        this.$buefy.toast.open('Employee successfully deleted!');
        ...
        async onUpdatePhoto(){
            try {
                ...
                // Remove the token that is returned in the url. A new one will be generated before each subsequent request
                let baseUrl = uploadBlobResponse._response.request.url.split('?')[0];

                // Save the blob image url to CosmosDB
                await addImageUrlToDB(this.employee.id, baseUrl);
